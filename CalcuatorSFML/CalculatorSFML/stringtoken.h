...
void StrCpy(string Input, string &token, int &pos, int notpos)
{
    for(int i = pos; i<notpos; i++)                                 //This for loop goes through the second cstring
    {
        token[i-pos]=Input[i];                                      //This copys what is in the second cstring into the first cstring
    }
}
